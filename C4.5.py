...
class C45DecisionTree:
    ...
    def _choose_best_feature(self, X, y):
        ...
        if best_gain_ratio < self.min_gain:
            return None, None
    ...
    def _gain_ratio(self, X, y, feature, threshold):
        ...
        intrinsic_value = self._intrinsic_value(X, feature, threshold)
    ...
    def _intrinsic_value(self, X, feature, threshold):
        ...
        if n_left == 0 or n_right == 0:
            return 0
        n_total = len(X)
        intrinsic_value = -(n_left / n_total) * np.log2(n_left / n_total) - (n_right / n_total) * np.log2(n_right / n_total)
...
clf = C45DecisionTree(min_samples_split=2, min_gain=0.0)
